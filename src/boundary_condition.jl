...
"""
function boundary_conditions_new(
    P::Float64,
    T::Float64,
    V::Float64,
    rho_m::Float64,
    ...
    eps_x_in = crystal_fraction_eps_x(
        param.composition, T_in, P_lit, tot_h2o_frac_in, tot_co2_frac_in
    )
    ...
    c_in =
        (
            (1 - eps_g_in - eps_x_in) * rho_m * param.c_m +
            eps_g_in * rho_g_in * c_g_in +
            eps_x_in * rho_x * param.c_x
        ) / rho_in
    if param.fluxing
        c_in = c_g_in
    end
    ...
    if sw.heat_cond == 1
        # heat loss
        Q_out = heat_conduction_chamberCH(
            param.maxn,
            a,
            cc,
            dr,
    ...
    Trt = heat_conduction_chamber_profileCH(
        param.maxn, a, cc, quadpts_r, param.kappa, param.Tb, param_saved_var
    )
    if param.rheol == "new"
