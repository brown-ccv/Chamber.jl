...
function heat_conduction_chamberCH(
    ...
    elseif time_index > 2 && time_index > lengthTime
        ...
        for n in 1:maxn
            ...
            temp =
                storeSumk[n] *
                exp(-kappa * (n * pi / c)^2 * (current_time - time[time_index - 1]))
            past_time = time[time_index - 1]
            past_time_delta = time[time_index]
            mk =
                (Tr0[time_index] - Tr0[time_index - 1]) /
                (time[time_index] - time[time_index - 1])
    ...
    elseif time_index > 2 && time_index == lengthTime
        ...
        for n in 1:maxn
            if switch_Tprofile == 0
                temp =
                    storeSumk_2_old[n] *
                    exp(-kappa * (n * pi / c)^2 * (current_time - time[end - 1]))
    ...
    for n in 1:maxn
        termn =
            sin(n * pi * (r - a) / c) *
            exp(-kappa * (n * pi / c)^2 * current_time) *
            (
                (a * (a + c) * Ta - a * (a + c) * Tb) / (n * pi) * (1 - cos(n * pi)) +
