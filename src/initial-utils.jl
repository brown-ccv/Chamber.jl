...
# For function eos_g
f_rho_g(P::Float64, T::Float64)::Float64 = real(
    1e3 * (
...
EosG(P, T) = EosG{typeof(P)}(f_rho_g(P, T), f_drho_g_dP(P, T), f_drho_g_dT(P, T))

struct EosG_RhoG{T}
    rho_g::T
end
EosG_RhoG(P, T) = EosG_RhoG{typeof(P)}(f_rho_g(P, T))
